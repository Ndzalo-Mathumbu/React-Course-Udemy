...
function SalesChart({ bookings, numDays }) {
  ...
  const data = allDates.map((date) => {
    // Get all bookings created on this date
    ...
    return {
      ...
      // Sum of all booking sales for the day
      totalSales: dayBookings?.reduce(
        (accum, booking) => accum + booking.totalPrice,
        0,
      ),

      // Sum of all extras sales for the day
      extrasSales: dayBookings?.reduce(
        (accum, booking) => accum + booking.extrasPrice,
        0,
      ),
    };
  }); */
  ...
  return (
    <StyledSalesChart>
      <Heading as="h2">{`Sales from ${fakeData[0]?.label} - ${fakeData.at(-1)?.label}`}</Heading>
